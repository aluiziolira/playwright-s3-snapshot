...
from .config import create_sample_config_file, load_config_manager
...
def parse_urls_from_file(file_path: str) -> list[str]:
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(f"URL file not found: {file_path}") from None
    except Exception as e:
        raise argparse.ArgumentTypeError(f"Error reading URL file: {e}") from None


def main() -> int:
    """Enhanced CLI with comprehensive validation and features."""
    # Load configuration first
    config = load_config_manager()
...
Configuration:
  ...
  - Environment variables (PS3S_BUCKET, PS3S_PREFIX, etc.)
    ...
    url_group.add_argument(
        "url", nargs="?", type=validate_url, help="URL to screenshot"
    )
    url_group.add_argument(
        "--url-file",
        type=parse_urls_from_file,
        metavar="FILE",
                ...
                try:
                    if args.bucket:
                        ...
                        if Path(result_path).exists():
                            ...
                            log_info("✅ Screenshot file created successfully!")
                        else:
